...
for epoch in range(epochs):  # loop over the dataset multiple times
    ...
    for i, data in enumerate(trainloader, 0):
        ...
        if (epoch % 4) == 0:
            ...
            cropped_inputs = inputs[:, :, :, 0:4096]
            cropped_inputs[:, :, :, cols * safe_sign:cols * (safe_sign + 1)] = \
                cropped_inputs[:, :, :, cols * safe_sign:cols * (safe_sign + 1)] * 0
        else:
            cropped_inputs = inputs[:, :, :, (cols * (epoch + 1)):((packets * cols) + (cols * (epoch + 1)))]
