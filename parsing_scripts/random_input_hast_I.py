...
def random_input_hast_I(folder_name, vector_length, rows, cols, prob):
    ...
    out_mat = np.zeros((vector_length, 1, rows, cols * depth), dtype=np.float32)
    ...
    for s in range(len(class_labels)):
        ...
        if class_labels[s] == 0:
            out_mat[s, 0] = np.random.rand(rows, cols * depth).astype(int) * 127
        else:
            out_mat[s, 0] = (np.random.rand(rows, cols * depth).astype(int) * 127) + 127
